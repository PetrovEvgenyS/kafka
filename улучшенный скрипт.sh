...
CONTROLLER_QUORUM="1@10.100.10.1:9093,2@10.100.10.2:9093,3@10.100.10.3:9093"
CONTROLLER_QUORUM_BOOTSTRAP="10.100.10.1:9093,10.100.10.2:9093"
...
installing_dependencies() {
    ...
    if ! java -version &>/dev/null; then
        errorprint "Java не установлена или не найдена в PATH!"
        exit 1
    fi
}
...
downloading_unpacking_kafka() {
    ...
    if [ $? -ne 0 ]; then
        errorprint "Ошибка скачивания Kafka!"
        exit 1
    fi
...
generation_cluster_id() {
    ...
    if [ ! -f "$CLUSTER_ID_FILE" ]; then
        if [ "$NODE_ID" -eq 1 ]; then
            ...
            errorprint "Ошибка: Cluster ID должен быть предварительно создан на Node 1 и скопирован на эту ноду!"
            exit 1
        fi
    fi
...
start_enable_kafka() {
    ...
    systemctl restart kafka || systemctl start kafka
}

# Функция проверки состояния Kafka:
check_status_kafka() {
...
copy_cluster_id() {
    if [ "$NODE_ID" -eq 1 ]; then
        ...
        echo "for i in {2..3}; do scp $INSTALL_DIR/cluster.id vagrant@10.100.10.\$i:$INSTALL_DIR/cluster.id; done"
    fi
}
